...
//Criação das classes
var Guiche = function Guiche(livre) {
  _classCallCheck(this, Guiche);

  this.livre = livre;
  this.tempoOcupado = 0;
};

var Cliente = function Cliente(horaEntrada) {
...
function startSimulation(guiches, expediente, tempoSaque, tempoDeposito, tempoPagamento) {
  ...
  var filaClientes = new Array(); //Variáveis que controlam a duração do expediente

  var tempo = 0,
      tempoExtra = 0; //Início do expediente

  while (tempo <= tempoExpediente || filaClientes.length != 0) {
    if (tempo > tempoExpediente) {
      //Tempo Extra
      tempoExtra++;
    }

    if (tempo <= tempoExpediente) {
      //Impede a entrada de clientes depois do fim do expediente
      if (chegouCliente()) {
    ...
    if (guicheLivre(listaGuiches, qtdGuiche) && filaClientes.length > 0) {
      ...
      for (var i = 0; i < qtdGuiche; i++) {
        //Verifica qual guiche está livre
  ...
  tempoExtra /= 60;
  minExtra = Math.round(tempoExtra % 60); //Resultados
